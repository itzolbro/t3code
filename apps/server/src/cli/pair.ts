...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      ...
      "Start one with `npx t3 serve`, then pair from this machine's browser or another device.",
    ].join("\n");
  }
}
 ...
 * server (do NOT overwrite its mapping).
 */
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      if (isLoopbackHost(new URL(pairingBaseUrl).hostname)) {
        notes.push(
          "This URL is only reachable from this machine. Restart the server with a reachable --host to pair remotely.",
        );
      }
