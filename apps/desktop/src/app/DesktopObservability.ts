...
const backendOutputLogFactoryLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // a path collision (e.g. two instance ids that sanitize to the
    // same file name) doesn't attribute the second caller's writes to
    // the first caller's id. Each sink pins itself to the factory's
    // scope so all log resources tear down together at app exit.
    // Mutex serializes concurrent first-time lookups for the same
    // file path.
